...
impl Address {
    ...
    pub fn new_from_mnemonic(mnemonic: &str) -> Result<Self, &'static str> {
        // 解析助记词
        let mnemonic =
            Mnemonic::parse_in(Language::English, mnemonic).map_err(|_| "Invalid mnemonic")?;

        // 生成种子，使用空密码
        let seed = Seed::new(mnemonic.to_seed(""));

        // 从种子生成主私钥
        let root_key = XPrv::new(seed).map_err(|_| "Failed to derive root key")?;
    ...
    pub fn generate_payment_code(mnemonic: &str) -> Result<String, &'static str> {
        ...
        let seed = Seed::new(mnemonic.to_seed(""));
        let root_xprv = XPrv::new(seed).map_err(|_| "Failed to derive root key")?;

        let path = DerivationPath::from_str(&format!("m/47'/{}'/0'", FAIC_COIN_TYPE))
            .map_err(|_| "Invalid derivation path")?;

        let payment_code_xprv = path
            .into_iter()
            .try_fold(root_xprv, |key, child_number| {
    ...
    pub fn new_segwit_address(mnemonic: &str) -> Result<Self, &'static str> {
        ...
        let address =
            segwit::encode(hrp, Fe32::Q, hash.as_ref()).map_err(|_| "Failed to encode address")?;
