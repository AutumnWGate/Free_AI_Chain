...
impl MerkleTreeManager {
    ...
    /// 从交易列表构建默克尔树
    pub async fn build_merkle_tree(&mut self, transactions: &[Transaction]) -> Result<Hash, LedgerError> {
    ...
    async fn save_merkle_root(&self, root: &Hash) -> Result<(), LedgerError> {
        ...
        sqlx::query(
            "INSERT INTO merkle_roots (root_hash, timestamp) VALUES ($1, $2)"
        )
    ...
    /// 生成交易的默克尔证明
    pub async fn generate_proof(&self, transaction: &Transaction) -> Result<MerkleProof, LedgerError> {
    ...
    /// 保存默克尔证明到数据库
    async fn save_merkle_proof(&self, transaction: &Transaction, proof: &MerkleProof) -> Result<(), LedgerError> {
        debug!("正在保存默克尔证明到数据库");
        
        let proof_data = serde_json::to_string(&proof)
            .map_err(LedgerError::SerializationError)?;
