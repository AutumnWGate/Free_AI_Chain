...
impl WalletManager {
    ...
    pub async fn create_wallet(&self) -> Result<Wallet, LedgerError> {
        info!("正在创建钱包...");
        let (wallet, mnemonic) =
            Wallet::create_wallet().map_err(|e| LedgerError::WalletError(e.to_string()))?;
    ...
    /// 获取钱包管理信息
    pub async fn get_wallet_management(
        &self,
        addresses: Option<Vec<String>>,
    ) -> Result<WalletManagement, LedgerError> {
        debug!("正在获取钱包管理信息...");
            ...
            for action in &wallet_management.actions {
                match action {
                    ...
                    WalletAction::GetTransactionHistory { address } => {
                        ...
                        self.db_ops
                            .update_transaction_history(address, &transactions)
                            .await?;
                    }
                }
            }
