...
impl BlockManager {
    ...
    pub async fn create_block(
        ...
        // 计算区块哈希
        let block_hash =
            calculate_block_hash(&header).map_err(|e| LedgerError::BlockError(e.to_string()))?;
    ...
    pub async fn verify_block(&self, block: &Block) -> Result<bool, LedgerError> {
        ...
        let computed_hash = calculate_block_hash(&block.header)
            .map_err(|e| LedgerError::BlockError(e.to_string()))?;
        ...
        // 3. 验证区块签名
        let block_bytes =
            serde_json::to_vec(&block.header).map_err(|e| LedgerError::SerializationError(e))?;

        if !block
            .header
            .signature
            .verify(&block_bytes, &block.header.validator)
    ...
    pub async fn verify_block_merkle_tree(&self, block: &Block) -> Result<bool, LedgerError> {
        ...
        // 验证默克尔树根
        let computed_root = merkle_manager
            .verify_merkle_root(&block.transactions, &block.header.merkle_root)
            .await?;
