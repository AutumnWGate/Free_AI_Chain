...
impl Wallet {
    ...
    pub fn create_wallet(password: &str) -> Result<(Self, String), WalletError> {
        ...
        // 使用密码生成助记词
        let mnemonic = mnemonic::generate_from_password(password).map_err(|e| {
            error!("生成助记词失败: {}", e);
    ...
    pub fn recover_mnemonic(&self, password: &str) -> Result<String, WalletError> {
        ...
        let encrypted_mnemonic = self.encrypted_mnemonic.as_ref().ok_or_else(|| {
            error!("找不到加密的助记词");
    ...
    pub async fn get_full_transaction_history(
        ...
        ops.get_transaction_history_paged(&self.address.to_string(), offset, page_size)
            .await
            .map_err(|e| WalletError::TransactionHistoryError(e.to_string()))
    }
}
