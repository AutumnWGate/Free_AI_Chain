...
impl Transaction {
    ...
    pub fn verify_signature(&self) -> Result<bool, TransactionError> {
        ...
        // 2. 验证签名
        let is_valid = self.signature.verify(&message, &self.from)
...
impl Default for Transaction {
    fn default() -> Self {
        Transaction {
            ...
            nonce: 0,
            signature: SignatureWrapper::from_bytes(&[0u8; 65])
                .expect("默认签名应该总是有效的"), // 这里使用 expect 因为我们知道这是一个有效的默认值
            timestamp: Utc::now(),
            fee: Amount::default(),
            transaction_hash: Hash::new(),
        }
...
impl TransactionPool {
    ...
    pub fn contains(&self, transaction_hash: &Hash) -> bool {
        self.transactions.iter().any(|tx| &tx.transaction_hash == transaction_hash)
    }
